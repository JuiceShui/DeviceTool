...
public class SignalStrengthsHandler {
    ...
    private SignalStrengthsHandler() {
        mSubscriptionManager = SubscriptionManager.from(App.get());
        mTelephonyManager = (TelephonyManager) App.get().getSystemService(Context.TELEPHONY_SERVICE);
        initListeners();

        mSimStateReceiver = new SimStateReceive();
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(SimStateReceive.ACTION_SIM_STATE_CHANGED);
        App.get().registerReceiver(mSimStateReceiver, intentFilter);
    }
        ...
        if (null != mInstance) {
            synchronized (mLock) {
                if (null != mInstance) {
                    ...
                    App.get().unregisterReceiver(mSimStateReceiver);
                    mInstance = null;
                }
            }
        }
    ...
    private void listenSimSignalStrengths(SimCard simCard) {
        if (simCard == SimCard.SIM_CARD_1) {
            @SuppressLint("MissingPermission") SubscriptionInfo sub0 = mSubscriptionManager.getActiveSubscriptionInfoForSimSlotIndex(INDEX_SIM1);
            ...
            @SuppressLint("MissingPermission") SubscriptionInfo sub1 = mSubscriptionManager.getActiveSubscriptionInfoForSimSlotIndex(INDEX_SIM2);
